    ...
    require.async(['jquery', 'contabs'], function() {
        ...
        var columns = [{
            ...
            formatter: function(value, row, index) {
                ...
                opt.push('<a href="shop/cate-next.html?u=' + row.catId + '&n=' + row.name + '" data-index="cat-next-' + index + '"  class="btn btn-outline btn-info js-jiesuan J_menuItem">查看二级类别</a>');
                return opt.join(" ");
            },
            events: {
                'click .js-update': function(e, value, row, index) {
                    ...
                    $("#js-update-img-show").html("<div class='expmpleBox'><img id='js-update-img' data-or data-value='" + row.iconImg + "' src='" + row.iconImgPath + "?x-oss-process=image/resize,w_100'/></div>");
                    $("#updateModal").modal({
                        backdrop: 'static'
                    });
                },
                'click .js-delete': function(e, value, row, index) {
                    $alert._warning("确认删除该类别？", "该操作将删除一级商家类别\"" + row.name + "\"", function() {
                        $.ajax({
                            url: "/xyshop-supplier/shop-categroy/del",
        ...
        var $t = t._init("table", "/xyshop-supplier/shop-categroy/pagelist", columns);

        var imgUP = null,
            imgUpdateUP = null;
        $('#addModal').on('shown.bs.modal', function(e) {
            imgUP = $upload._init(imgUP, '#js-add-img-up', function(file, response) {
                $("#js-add-img-show").html("<div class='expmpleBox'><img  id='js-add-img'  data-value='" + response.value + "' src='" + response.url + "?x-oss-process=image/res100,w_100'/></div>");
            });
        });
        ...
        $(document).on("click", "#js-add-btn", function(e) {
            ...
            $.ajax({
                url: "/xyshop-supplier/shop-categroy/save-lv1",
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                type: "post",
                dataType: "json",
                data: {
