        ...
        function($http, $q, $ionicPlatform, $cordovaCamera, $cordovaToast, $cordovaDatePicker, $cordovaContacts, base) {
            var account = {
                ...
                reload: function() {
                    var deferred = $q.defer(),
                        self = this;
                    base.request("usreload", {userid: self.getUser().usersid })
                        .then(function(resp) {
                            if (resp.state == "ok") {
                ...
                sendCode: function(arg, effect) {
                    ...
                    var deferred = $q.defer();
                    base.request("ussendCode", {'phone': arg, 'action': effect})
                        .then(function(resp) {
                            deferred.resolve(resp);
                        }, function(resp) {
                            deferred.reject(resp);
                ...
                mychilds: function(arg) {
                    var deferred = $q.defer();
                    arg.usersid = this.getUser().usersid;
