define(function(require) {
	require.async(['jquery','icheck'], function() {
		...
		var columns=[{
            ...
            field: 'goodsAttrInfo', title: '规格',  align: 'center',
        },{
            ...
            field: 'shipInfo', title: '配送地址信息',  align: 'center'
        },{
            ...
            formatter:function(value, row, index){
            	switch (value) {
					...
					case 'score':return "<span class='badge badge-warning'>积分支付</span>";
		...
		var $ordert = ordert._init("ordertable","/shop-orders/order/unionlist",columns,function(d){
...
//				d.userName=$('#js-user-name').val();
			...
			$alert._warning("确认信息填写无误？","该操作将修改订单\""+$('#js-order-no').val()+"\"的物流信息为："+wuliuName+","+wuliuno,function(){
        		$.ajax({
					...
					async:true,success:function(data){
						...
						console.log($common._paramsEmpty($(this)));
						console.log($(this));
		...
		//发送模板消息
        var templateNofity = function(){
        	var url = $('#wx_domain').val();
            $.ajax({
        	   async:false,
        	   url: url + 'notify/shipments.html?orderUuid=' + $("#js-order-uuid").val(),
        	   type: "get",
        	   dataType: 'jsonp',
        	   jsonp: 'callback',
